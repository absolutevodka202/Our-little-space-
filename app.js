...
function render(rows){
 ...
 $("lettersList").innerHTML=g.letter.map(r=>`<article class="paper"><h3>${esc(r.title)}</h3><div class="meta">${esc(r.author)} · ${date(r.created_at)}</div><div class="content">${esc(r.content)}</div>${replies(r)}</article>`).join("")||empty("No letters yet. ♡");
 $("diaryList").innerHTML=g.diary.map(r=>`<article class="diary-entry"><h3>${esc(r.title)}</h3><div class="meta">${date(r.created_at)} · ${esc(r.author)}</div><div class="content">${esc(r.content)}</div>${replies(r)}</article>`).join("")||empty("The first page is waiting. ♡");
 $("memoriesList").innerHTML=g.memory.map(r=>`<article class="memory-card"><div class="memory-image">${r.media_url?`<img src="${esc(r.media_url)}">`:"♡"}</div><h3>${esc(r.title)}</h3><div class="meta">${date(r.created_at)} · ${esc(r.author)}</div><div class="content">${esc(r.content)}</div>${replies(r)}</article>`).join("")||empty("No memories yet. ♡");
 $("voiceList").innerHTML=g.voice.map(r=>`<article class="voice-card"><h3>${esc(r.title)}</h3><div class="meta">${date(r.created_at)} · ${esc(r.author)}</div>${r.media_url?`<audio controls src="${esc(r.media_url)}"></audio>`:""}<div class="content">${esc(r.content)}</div>${replies(r)}</article>`).join("")||empty("No voice notes yet. ♡");
 $("littleList").innerHTML=g.little.map(r=>`<article class="little-card"><h3>☆ ${esc(r.title)}</h3><div class="meta">${date(r.created_at)} · ${esc(r.author)}</div><div class="content">${esc(r.content)}</div>${replies(r)}</article>`).join("")||empty("Add a little thing that reminds you of them. ♡");
